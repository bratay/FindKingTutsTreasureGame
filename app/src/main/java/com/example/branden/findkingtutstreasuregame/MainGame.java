...
public class MainGame extends AppCompatActivity {
    ...
    int charNum = 0;
    int playerHP = 5;
    int playerHunger = 0;//if hunger gets to 5 player loses 1 hp after 2 actions if hunger == 6 2 hp lose, and so on
    int actionCounter = 0;//every action will raise this by 1, every 3 action hunger will raise by one
    int playerAttack = 1;
    int playerdefense = 4;

    int [][][][] map = new int[3][3][3][3];//this is the map when the
    //the player choosing a direction is 0,1, or 2 in each dimenition in the array
    //when they get to the 5th dimition they will start back at the first
    //the treasrue will be stored in a random index in the 5D array
    //all 243 indexes will be set to 3 the indexs the player is at will be 1 and the treasure will be at 2
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        charNum = start.getIntExtra(CharacterCreation.charactNum, 0);
        charName = start.getStringExtra(CharacterCreation.name);
    ...
    public void openInventory(View V) {
        ...
        //data that will be set to inventory if the user wants to look at there stats or save the game
    ...
    public void win(View V){
        Intent winner = new Intent(this, playerWins.class);
